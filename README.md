...
- **L4 (51.5", 20°)**: the top of travel is clear only at ≤ 22.5° and the 100°→20° rotation is impossible up there, so the carriage rises to the **33" station** (where 25–100° are all clear) and the arm starts rotating **as the carriage arrives** — the gate is the window, not a settled height. From there the two run **together** up to the **48" pre-top height**, the carriage climbing behind the descending arm; the last three and a half inches wait for the arm to **actually reach 22.5°** — that gate is compared strictly, not with the 3° tolerance the travel gates use, which would have opened it at 25.5°. That last gate is not negotiable: the corridor table says 25° is clear to 51", and on the robot it is not — letting the carriage climb past 48" with the arm still at 25° put the claw into the top bar. Leaving for a low pose mirrors it: down to 39" at the L4 angle, to the **33" station** once the arm is past 40°, and below that once the arm clears band A at 75° — each step continuous into the next, with no stop between them. The height itself is 51.5" rather than 52.5": half an inch under the forward soft limit left the carriage fighting the top of travel, and 52.5 sat exactly on the 20° corridor ceiling. Raise it toward 52 if the coral needs the height, not past it.
...
- **L3 ↔ L4 goes straight across.** Both poses live in the upper corridor and their bands overlap over **36–51"**, so the arm turns only the five degrees between 25° and 20°, at the near edge of that overlap: climbing, it turns at **36"** and keeps climbing behind the ratchet; descending, it turns at **48"** and keeps dropping. The plan used to swing the arm out to RAISE and back — 150° of rotation to avoid five — and on the way up it also let the carriage dive toward the base before climbing again. Simulated with the profiles the robot runs, the transfer is **0.92 s up** (was 1.75 s) and **0.90 s down** (was 1.56 s), with nothing standing still in either.
