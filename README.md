...
| Elevator – Cruise Velocity (in/s) / Max Acceleration (in/s²) | 18 / 120 | MAXMotion profile (full travel in ~3.0 s). 18 in/s is the practical ceiling on the 45:1 gearing — see *Match-pace profiles* below. The Superstructure handoff heights follow these automatically — re-check the *Handoffs* readout after a change |
| Pivot – Cruise Velocity (deg/s) / Acceleration (deg/s²) / Jerk (deg/s³) | 240 / 480 / 4800 | CorAl Motion Magic profile, re-applied to the TalonFX the next time the robot is disabled (90° swing ≈ 0.9 s). If a coral slips during a swing, lower the acceleration first. Handoff heights follow these too |
...
| Superstructure – L3 / L4 Arm Arrival Offset (s) | +1.15 / 0.0 | Seconds after the elevator settles that the arm finishes its rotation (negative = early). The handoff *heights* are derived from these and the motion profiles — see the Testing tab's *Handoffs* readout. L4 ships at 0.0 (arm finishes as the elevator settles); go negative only after watching L4 |
...
4. **Tune closed-loop gains** (elevator kP/kS/kV/kG and both mechanisms' profiles from the Testing-tab tunables — they re-apply the next time the robot is disabled; pivot kP/kS/kV/kG via Phoenix Tuner X; `AutoConstants` path-following kP) and test autos. Both mechanisms now ship at **match-pace profiles** (elevator 18 in/s / 120 in/s², pivot 240°/s / 480°/s² with a jerk limit and a 60 A stator cap) after the gentle first-power-on values tracked cleanly. First moves at these speeds: watch L4 (the arm now starts its rotation at ≈41" — see the *Handoffs* readout) and a held coral through a fast swing; if it slips, lower the pivot *Acceleration* first. The elevator's 18 in/s is the practical ceiling on the 45:1 gearing (≈80% of the NEO's free speed, ≈10 V at cruise); a shorter gearbox (25:1 or 27:1 cartridges) is the hardware path to a faster elevator and needs only `ELEVATOR_GEAR_RATIO` changed in code.
