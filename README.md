...
| Elevator – Cruise Velocity (in/s) / Max Acceleration (in/s²) | 16 / 60 | MAXMotion profile (full travel in ~3.6 s); the Superstructure handoff heights follow these automatically — re-check the *Handoffs* readout after a change |
...
5. Repeat the 20" move — dashboard and tape should agree within ⅛". Then run L4 (52.5") once before touching *Cruise Velocity* / *Max Acceleration*.
...
4. **Tune closed-loop gains** (elevator kP/kS/kV/kG and profile from the Testing-tab tunables — they re-apply the next time the robot is disabled; pivot kP/kS/kV/kG via Phoenix Tuner X; `AutoConstants` path-following kP) and test autos. The shipped pivot profile is a deliberately **gentle first-power-on value** (60°/s with a jerk limit and a 60 A stator cap) so nothing can slam while gains are dialed in — raise it in `Constants.java` once holding and tracking are clean (targets are noted in the comments). The elevator ships at 16 in/s / 60 in/s² (the 8 in/s first-power-on profile was too slow on the robot); adjust it with the *Cruise Velocity* / *Max Acceleration* tunables, no redeploy.
