...
2. **Refine the elevator travel ratio** with a long move: command 20" from zero, tape the carriage, and set `ELEVATOR_MEASURED_TRAVEL_RATIO = measured ÷ 20`. The current 1.375 came from a 2" move, where tape error is ~5%; a 20" move gets it to ~0.5%, which matters for L4 (52.5") landing on the branch.
