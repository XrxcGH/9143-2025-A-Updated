...
**Latest release:** [v1.0](https://github.com/XrxcGH/9143-2025-A-Updated/releases/tag/v1.0) has the release notes and source archives; all releases are on the [Releases page](https://github.com/XrxcGH/9143-2025-A-Updated/releases).

---

## Robot overview

- **Swerve drivetrain**: CTRE Phoenix 6 swerve on a 30"×30" frame with mixed modules (front: SDS MK4i, back: SDS MK4n), all with L3+ drive gearing, all Kraken X60 drive/steer, CANcoders, and a Pigeon 2. Drive/coupling ratios are uniform; only the steer ratios differ and are set per module.
...
- **LED system** (code commented out, because there is no CANdle on the robot): CTRE CANdle robot-state patterns, kept in [LEDs.java](src/main/java/frc/robot/subsystems/LEDs.java) for when one is installed.
...
- **Setpoints are sent once.** MAXMotion regenerates its profile from the *measured* state every time a setpoint arrives, so `Elevator.setPosition` never re-sends the setpoint it is already running. `Elevator/Setpoint Count` on the dashboard must not move while the carriage is only holding.
...
One real cost of the light gearing: with the robot disabled at height, brake mode lets the carriage creep down ≈ 3 in/s, so stow before disabling.
...
- **Low box.** Any arm angle from 8° up is clear below 16.5", so base to L2 and base to L1 move both mechanisms together. Tucked (< 8°) is allowed only below 8.5" (the cross bar under the top sprockets).
- **Climbing out** needs the arm at ≥ 75° (the middle-stage top tube blocks roughly 18-30" for lower angles), and from RAISE = 100° the carriage may go anywhere. 90° is not a travel angle: the claw's lower rear meets the middle-stage top tube from ≈ 36" up. Ascents give the carriage a ratcheting head start (8.5" while tucked, then 16.5" once clear, then 36" past 75°, then anything once the arm reads RAISE).
...
|---|---|---|---|
| BASE / L1 / L2 to L3 | FULL | 1.5-1.6 s | one run, never below cruise |
| BASE / L1 / L2 to L4 | BRISK | 1.8-2.0 s | one run at 32 in/s (FULL is 1.66 s but only *barely* hitch-free; a real arm lags its profile) |
| L3 to L4, either way | FULL | 0.7 s | one run |
| L4 to BASE / L2 | FULL to 41.5", then SLOW to the station, FULL below | 2.0 s | never below ≈ 7 in/s (FULL throughout takes the same 2.0 s with two dead stops) |
| algae pose to L4 | EASY | 1.5-1.9 s | the arm has 140° to come round; the carriage still waits at 35.5" |
...
| Capture resolution / FPS | The lowest mode that still detects tags across the field; 640×480 at 90 fps is plenty |
...
| Camera exposure/gain | Shorter exposure lowers sensor heat slightly; keep enough to detect tags reliably |
...
| Autonomous | Auto selection and monitoring | Auto chooser, big match timer, Field widget with live robot pose, game piece indicator, Alerts |
...
Every tab fits 12 × 5 grid cells (1536 × 640 px at grid size 128), which leaves room for a docked Driver Station on the drive laptop. To change the layout, edit it in Elastic, `File > Export Layout`, and commit the exported file back to `src/main/deploy/elastic-layout.json`. The robot switches Elastic to the right tab automatically on mode changes (Setup when disabled, Autonomous in auto, Teleop in teleop, Testing in test) via ElasticLib ([util/Elastic.java](src/main/java/frc/robot/util/Elastic.java)). The Zero buttons on the Setup tab act only while the robot is disabled.
...
| `Vision/Visible Tags` | Every tag ID per camera, for example `funnel: 12 \| reef: 18 19` (Setup tab, under the camera streams) |
...
2. Arm at 100° (Pivot Setpoint 100, then Pivot Go). Testing tab: Elevator Setpoint 20, then Elev Go. Let it settle, then read the dashboard height and the tape.
3. If the dashboard reads 20.0 and the tape is off by the same amount at 20" and 40", the hard-stop height is wrong; adjust it by the difference. If the tape is off by a percentage (twice as far off at 40" as at 20"), set *Elevator - Travel Ratio* = current × tape ÷ commanded; it should stay at 1.0.
...
The articulated robot model lives in [advantageScopeAssets/Robot_Leviathan](advantageScopeAssets/Robot_Leviathan). The repository carries its `config.json` and a README with the component order; the model binaries are not in the repository (the robot's CAD is not distributed, and the folder's `.glb` / `.step` files are git-ignored). Export your own from the CAD as four glTF binaries: `model.glb` (static chassis), `model_0.glb` (middle stage), `model_1.glb` (carriage) and `model_2.glb` (arm). Drop them into that folder. Then, in AdvantageScope's 3D Field tab (one-time, per laptop):
...
Leaving L3 / L4 puts the claw 9-12" past the front bumper, so a stow in place against the reef swings it into the reef. That is why the match autos use `prep*`, then `ejectCoral`, then `stowAfterBackingOff` rather than the in-place `score*` commands.
...
- `Demo.traj` is a placeholder straight-line move (from 2 m to 3 m on X) so the feature is testable out of the box (`ChoreoTrajectoryTest` loads it through the real parser). Replace it with real trajectories.
...
| Swerve steering axes | all four at 12.375" from center: 2.625" inset for the MK4n too, not only the MK4i | matches `TunerConstants` (±12.375") |
| Elevator sprocket | "25 Chain 22t .500 Hex Sprocket" | matches (22T) |
| Elevator hard-stop height | base bottom cross tube top 5.875", middle-stage 1" tube on it, carriage tube bottom 6.875", so 1.000" | 1.000", also confirmed on the robot; the clearance map is computed at the same rest height |
...
5. Deploy (`./gradlew deploy` or the WPILib "Deploy Robot Code" command), then load the dashboard layout: in Elastic, `File > Load Layout From Robot`.
